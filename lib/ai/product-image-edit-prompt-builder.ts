...
export function buildProductImageEditPrompt({ goalId, outputSettings, userPrompt }: ProductImageEditPromptInput) {
  ...
  return [goal.promptTemplate, outputSettingsPrompt, PRODUCT_PRESERVATION_RULES, PRODUCT_VISUAL_FIDELITY_RULES, BRAND_AND_AUTHORIZATION_RULES, extraPrompt ? `用户额外要求：${extraPrompt}` : ""]
    .filter(Boolean)
    .join("\n\n");
}
