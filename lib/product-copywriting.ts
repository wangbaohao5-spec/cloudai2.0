  ...
  return {
    ...
    sellingPoints:
      joinAnalysisItems(
        ...
        [analysis.material, analysis.color].filter(Boolean).length ? `材质颜色：${[analysis.material, analysis.color].filter(Boolean).join(" / ")}` : undefined,
        analysis.specifications ? `规格：${analysis.specifications}` : undefined,
